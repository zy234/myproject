...
from bert_data_load import get_attention_mask, MyDataSet
...
def eval_model_by_seqeval(model, input_ids, label_ids):
    model.eval()
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader):
            ...
            for index, sentence in enumerate(scores):
                # 截取有用的label
                num = torch.sum(attention_mask[index])
                score = scores[index][:num]
                label = labels[index][:num]

                t = mydataset.label_trans(score)
                p = mydataset.label_trans(label)
                true_label.append(t)
                pred_label.append(p)
    return classification_report(true_label, pred_label)
