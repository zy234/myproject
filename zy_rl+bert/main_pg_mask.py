...
from policy_gradient import PG, policy_gradient


def train(model, pg, iterator, optimizer, criterion, device):
    model.train()
    loss = 0
    for i, batch in enumerate(tqdm(iterator)):
        ...
        mask_x = policy_gradient(x, y, model, seqlens, pg)
...
if __name__=="__main__":
    ...
    parser.add_argument("--logdir", type=str, default="./result/bert+pg_mask")
    ...
    for epoch in range(hp.n_epochs):  # 每个epoch对dev集进行测试
        ...
        train(model, pg, train_iter, optimizer, criterion, device)
