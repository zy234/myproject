...
class Bert_BiLSTM_CRF(nn.Module):
    ...
    def _forward_alg(self, feats):
        ...
        for t in range(1, T):
            log_alpha = (log_sum_exp_batch(self.transitions + log_alpha, axis=-1) + feats[:, t]).unsqueeze(1)  #(128,8).unsqueeze(1)
    ...
    def neg_log_likelihood(self, sentence, tags):
        ...
        forward_score = self._forward_alg(feats)  # (bs, 1)
        gold_score = self._score_sentence(feats, tags)  #(bs, 1)
        return torch.mean(forward_score - gold_score)  # loss
